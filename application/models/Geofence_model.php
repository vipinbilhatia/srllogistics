...
class Geofence_model extends CI_Model{
	...
	public function add_geofence($data) { 
		if(isset($_POST['geo_notify_members'])) {
			$geo_notify_members = json_encode($_POST['geo_notify_members']);
		} else {
			$geo_notify_members = json_encode(array());
		}
		$insertdata = array('geo_name'=>$data['geo_name'],'geo_description'=>$data['geo_description'],'geo_area'=>$data['geo_area'],'geo_vehicles'=>implode(',', $data['geo_vehicles']),'geo_notify_type'=>$data['geo_notify_type'],'geo_notify_members'=>$geo_notify_members);
